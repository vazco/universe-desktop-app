  ...
  spawn(meteor_bin, ['create', '--example', 'leaderboard'], {
    ...
    spawn(meteor_bin, ['remove-platform', 'ios', 'android'], {
      ...
      cwd: leaderboard
       ...
       // adding desktop meteor package
      spawn(meteor_bin, ['add', 'universe:desktop-app'], {
        stdio: 'inherit',
        cwd: leaderboard,
        // modify env var so desktop package will know how to proceed,
...
target.dev = function(action){
  ...
  var leaderboard_desktop = path.join(leaderboard, '.desktop');

  log('starting in dev mode');

  if(~'reset'.indexOf(action))
    shell.rm('-rf', leaderboard_desktop);
...
function log(){
  ...
  console.log.apply(null, ['desktop: '].concat(args));
}
