...
function Env (input, output, settings, cli) {
    ...
    this.is_development_mode = (process.env.DEVDESKTOP == 'true');

    // is not running in test mode
    this.is_running_tests = (process.env.TESTDESKTOP == 'true');

    // default stdio config for child processes
    var log_levels = process.env.LOGDESKTOP || '';
    ...
    // .desktop folder will only exist in development mode, not after packaged
    if (/\.desktop$/m.test(this.app.root))
    ...
    this.app.config_path = join(this.app.root, 'desktop.json');
    ...
    else if (process.env.DESKTOP_SETTINGS_FILE)
        this.app.settings = require(process.env.DESKTOP_SETTINGS_FILE);
    else
        this.app.settings = settings || {};

    // internal electron distro
    this.core = {};
    this.core.tmp = join(this.os.tmp, 'desktop');
