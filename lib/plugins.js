...
function Plugins ($) {
    this.$ = $;
    this.log = require('./log')($, 'desktop:plugins');
    this._all = [];
...
Plugins.prototype.auto_load = function (done) {
    ...
    var config_path = path.join(this.$.env.app.root, 'desktop.json');
    ...
    _.each(config.plugins, function (plugin_name) {
        ...
        if (fs.existsSync(plugin_path))
            ...
            self.log.warn([
                ...
                '~> remember to run `npm install <plugin> --save` inside .desktop dir'
            ].join('\n'));
        }
    });
