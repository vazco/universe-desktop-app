...
function NodeJS ($, logger) {
    this.$ = $;
    this.log = require('../log')($, 'desktop:plugins:nodejs');
    this.name = 'nodejs';
    ...
    this.config = {
        ROOT_URL: 'http://localhost:' + ((this.$.env.app && this.$.env.app.config && this.$.env.app.config.port) || 3000) // find meteor's port in run time?
    };

    this.log = logger(this.$, 'desktop:plugins:' + this.name);
}
